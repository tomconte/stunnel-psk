...
// <tomconte>
unsigned int psk_callback(SSL *ssl, const char *identity, unsigned char *psk, unsigned int max_psk_len);
// </tomconte>
...
int context_init(SERVICE_OPTIONS *section) { /* init SSL context */
    ...
    // <tomconte>
    s_log(LOG_DEBUG, "PSK: set callback");
    SSL_CTX_set_psk_server_callback(section->ctx, psk_callback);
    // </tomconte>
...
// <tomconte>
/**************************************** PSK callback */
char *opt_psk="c033f52671c61c8128f7f8a40be88038bcf2b07a6eb3095c36e3759f0cf40837";
char my_psk[256];
size_t psk_len = 0;
...
void psk_unhexify() {
    ...
    {
        s_log(LOG_DEBUG, "PSK: not valid hex");
        return;
    }
...
unsigned int psk_callback(SSL *ssl, const char *identity, unsigned char *psk, unsigned int max_psk_len) {
    s_log(LOG_DEBUG, "PSK: callback psk=%s", identity);
    psk_unhexify();
    strncpy(psk, my_psk, psk_len);
    return psk_len;
}
// </tomconte>
